...
LOG_MODULE_DECLARE(dw1000, CONFIG_DW1000_LOG_LEVEL);

#define TX_WAIT_RESP_NRF52840_DELAY 30

#define DW_INST DT_INST(0, qorvo_dw1000)
#define DW_SPI DT_PARENT(DT_INST(0, qorvo_dw1000))
...
int dw1000_spi_init(void)
{
    ...
    if (!spi)
    {
        LOG_ERR("dw1000 SPI binding failed");
        return -1;
    }
    else
    {
        LOG_INF("dw1000 on %s (max %dMHz)", DT_PROP(DW_INST, label),
